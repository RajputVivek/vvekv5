...
(function () {
  ...
  document.body.addEventListener("mouseover", (e) => {
    if (
      e.target.closest(
        "a,button,.skill-card,.proj-card,.val-card,.stat-card,.principle-card,.demo-card,.tools-chips span,.soc-link,.btn,.cs-card,.cs-expand-btn",
      )
    ) {
...
/* ── SCROLL REVEAL ── */
(function () {
  const els = Array.from(document.querySelectorAll(".reveal-up, .reveal-left, .reveal-right")).filter(
    (el) => !el.closest("#hero"),
  );
...
(function () {
  try {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const EMAILJS_SERVICE_ID = "service_dgfsxuv";
    ...
    console.error("[contact] init error", initErr);
  }
})();
...
(function () {
  ...
  function updateDashboard(period) {
    ...
    animateNum(
      document.getElementById("dw-applied"),
      +document.getElementById("dw-applied").textContent.replace(/,/g, "") || 0,
      d.applied,
    );
    animateNum(
      document.getElementById("dw-assessed"),
      +document.getElementById("dw-assessed").textContent.replace(/,/g, "") || 0,
      d.assessed,
    );
    animateNum(
      document.getElementById("dw-shortlisted"),
      +document.getElementById("dw-shortlisted").textContent.replace(/,/g, "") || 0,
      d.shortlisted,
    );
    ...
    labels.forEach((l, i) => {
      l.textContent = vals[i].toLocaleString();
    });
  }
  ...
  const levels = [
    {
      ...
      max: 24,
      label: "Beginner",
      desc: "Just getting started. This is where every expert once stood — the important thing is to be curious and consistent.",
    },
    {
      min: 25,
      max: 49,
      label: "Learning",
      desc: "Building foundational knowledge. You can follow tutorials and understand basic patterns. Keep shipping small projects.",
    },
    {
      min: 50,
      max: 69,
      label: "Proficient",
      desc: "You understand the core concepts and can work independently on standard tasks. Ready for real team contribution.",
    },
    {
      min: 70,
      max: 84,
      label: "Advanced",
      desc: "You're comfortable with hooks, component patterns, and state management. You understand the why behind the how.",
    },
    {
      min: 85,
      max: 100,
      label: "Expert",
      desc: "Deep mastery. You can design systems, mentor others, debug edge cases, and make architectural decisions confidently.",
    },
  ];
...
(function () {
  ...
  if (likeBtn && likeCount) {
    ...
    likeBtn.addEventListener("click", () => {
      ...
      if (liked) {
        ...
        icon.animate([{ transform: "scale(1)" }, { transform: "scale(1.6)" }, { transform: "scale(1.3)" }], {
          duration: 380,
  ...
  if (copyBtn && copyIcon) {
    copyBtn.addEventListener("click", () => {
      navigator.clipboard
        .writeText("npm install vivek-ui")
        .then(() => {
