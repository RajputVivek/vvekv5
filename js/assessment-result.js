/* ═══════════════════════════════════════════════════════════
   SELECT10X — Assessment Result Page
   Full Interactivity + Mock Data for Omkar Almale
...
const QUESTIONS = [
  ...
  /* ══ 3. Coding — Correct ══ */
  {
    ...
    text: 'Implement a function <code>twoSum(nums, target)</code> that returns indices of the two numbers in the array that add up to target. Each input has exactly one solution.',
    lang: 'JavaScript',
    candidateCode: `<span class="tok-cmt">// Omkar's solution — O(n) time, O(n) space</span>
...
<span class="tok-punct">}</span>`,
  ...
  /* ══ 7. FITB — Correct ══ */
  {
    ...
    blanks: [
      {
        before: 'A closure is a function that has access to its outer scope even after the outer function has',
        candidate: 'returned', correct: 'returned', isCorrect: true,
        after: '.',
      },
      {
        before: 'The JavaScript event loop processes tasks from the',
        candidate: 'callback queue', correct: 'callback queue', isCorrect: true,
        after: 'after the call stack is empty.',
      },
      {
        before: 'Promises are resolved in the',
        candidate: 'microtask queue', correct: 'microtask queue', isCorrect: true,
        after: ', which has higher priority than the macrotask queue.',
      },
    ],
  },
...
<span class="tok-kw">export default</span> <span class="tok-var">useDebounce</span><span class="tok-punct">;</span>`,
  ...
  /* ══ 14. FITB — Correct ══ */
  {
    ...
    blanks: [
      {
        before: 'To create a 3-column grid with equal width columns, you would use:',
        candidate: 'grid-template-columns: repeat(3, 1fr)', correct: 'grid-template-columns: repeat(3, 1fr)', isCorrect: true,
        after: '.',
      },
      {
        before: 'The shorthand property for both row and column gaps in a grid is',
        candidate: 'gap', correct: 'gap', isCorrect: true,
        after: '.',
      },
    ],
  },
  ...
  <span class="tok-cmt">// Handle Arrays</span>
  <span class="tok-kw">if</span> <span class="tok-punct">(</span><span class="tok-type">Array</span><span class="tok-punct">.</span><span class="tok-fn">isArray</span><span class="tok-punct">(</span><span class="tok-var">obj</span><span class="tok-punct">))</span> <span class="tok-punct">{</span>
  ...
  <span class="tok-cmt">// Handle Date</span>
  ...
  <span class="tok-cmt">// Handle plain objects</span>
...
- Implement CSP headers to restrict which scripts can execute`,
  ...
  /* ══ 20. FITB — Correct ══ */
  {
    ...
    blanks: [
      {
        before: 'Node.js uses a single-threaded',
        candidate: 'event loop', correct: 'event loop', isCorrect: true,
        after: 'to handle concurrent operations.',
      },
      {
        before: 'The',
        candidate: 'libuv', correct: 'libuv', isCorrect: true,
        after: 'library provides Node.js with asynchronous I/O capabilities.',
      },
      {
        before: 'CPU-intensive tasks should be offloaded to',
        candidate: 'worker threads', correct: 'worker threads', isCorrect: true,
        after: 'to avoid blocking the event loop.',
      },
    ],
  },
...
const S = {
  activeTab:     'questions',
  qFilter:       'all',        // all | correct | wrong | partial
  typeFilter:    'all',
  searchQuery:   '',
  activeQNum:    null,
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initQuestionFilters();
  initPrintShare();
  // Analytics charts initialised lazily when tab first opens
  console.log('✓ Select10X Assessment Result loaded');
});
...
function initScoreRing() {
  ...
  // Insert gradient def into the SVG
  ...
  // Counter animation
  animateCounter(valEl, 0, targetScore, 1400);
}
...
function initKPICounters() {
  const kpis = document.querySelectorAll('.kpi-val[data-count]');
...
function renderQNavigator() {
  ...
  nav.innerHTML = QUESTIONS.map(q => {
    const tipMap = { mcq: 'MCQ', mrq: 'Multiple Response', coding: 'Coding',
      video: 'Video', tabular: 'Tabular', fitb: 'Fill in Blank', descriptive: 'Descriptive' };
    return `<button class="qnav-btn ${q.result}" data-qid="${q.id}" title="Q${q.id} · ${tipMap[q.type]}">
      ${q.id}
  ...
  nav.addEventListener('click', e => {
    ...
    if (qCard) {
      ...
      qCard.style.outline = '2px solid var(--blue)';
      setTimeout(() => { qCard.style.outline = 'none'; }, 1500);
    }
  });
}
...
function renderSkillBars() {
  ...
  container.innerHTML = SKILLS.map(s => `
    <div class="skill-bar-item">
      ...
      <div class="skill-bar-track">
        <div class="skill-bar-fill ${s.tier}" style="width:${s.pct}%"></div>
      </div>
    </div>`).join('');
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.querySelectorAll('.skill-bar-fill').forEach(f => f.classList.add('anim'));
...
function renderQuestions() {
  ...
  if (qs.length === 0) {
    list.innerHTML = `<div style="text-align:center;padding:48px 24px;color:var(--gray-400)">
      <i class="fa-solid fa-circle-question" style="font-size:2rem;margin-bottom:12px;display:block"></i>
...
function buildQuestionCard(q, idx) {
  const typeLabel = { mcq:'MCQ', mrq:'Multiple Response', coding:'Coding',
    video:'Video', tabular:'Tabular', fitb:'Fill in Blank', descriptive:'Descriptive' }[q.type];
  ...
  return `<div class="q-card ${q.result}" id="q-card-${q.id}" style="animation-delay:${idx*30}ms">
    <div class="q-card-head">
      <div class="q-num">${q.id}</div>
      <div class="q-head-meta">
        <div class="q-type-row">
          <span class="q-type-chip qtc-${q.type}"><i class="${typeIcon(q.type)}"></i> ${typeLabel}</span>
...
function buildMRQ(q) {
  ...
  const optionsHTML = q.options.map(opt => {
    ...
    if (isSelected && isCorrect) {
      ...
      cls = 'selected-wrong';
      note = `<span class="mrq-note extra">Extra selection</span>`;
    } else if (!isSelected && isCorrect) {
      cls = 'missed-correct';
      note = `<span class="mrq-note missed">Missed</span>`;
    }
  ...
  return `<div class="mrq-options">${optionsHTML}</div>
    <div class="mrq-breakdown">
      <span>✅ <strong>${selectedCorrect}/${totalCorrect}</strong> correct answers selected</span>
...
function buildFITB(q) {
  const sentencesHTML = q.blanks.map((b, i) => {
  ...
  const allCorrect = q.blanks.every(b => b.isCorrect);
  const correct = q.blanks.filter(b => b.isCorrect).length;

  return `${sentencesHTML}
    <div class="fitb-legend">
      <div class="fitb-legend-item">
        <span class="blank-slot correct" style="min-width:auto;padding:2px 8px;font-size:0.68rem"><i class="fa-solid fa-check"></i> Correct</span>
        Matches expected answer
      </div>
      <div class="fitb-legend-item">
        <span class="blank-slot wrong" style="min-width:auto;padding:2px 8px;font-size:0.68rem"><i class="fa-solid fa-xmark"></i> Wrong</span>
...
function buildDescriptive(q) {
  ...
  <div class="ai-rubric">
    <div class="rubric-head">
      <div class="rubric-head-left">
        <i class="fa-solid fa-list-check"></i> AI Scoring Rubric
      </div>
...
function initTabSwitcher() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (target === 'analytics' && !analyticsInited) {
        initAnalyticsCharts();
        renderDifficultyGrid();
        analyticsInited = true;
      }
      if (target === 'summary' && !summaryInited) {
...
function initQuestionFilters() {
  ...
  document.querySelectorAll('.qf-btn[data-qfilter]').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('.qf-btn[data-qfilter]').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      ...
      datasets: [
        {
          ...
          backgroundColor: sections.map(s => s.score >= 85
            ? 'rgba(22,163,74,0.85)' : s.score >= 70
            ? 'rgba(37,99,235,0.85)' : 'rgba(217,119,6,0.85)'),
          borderRadius: 6,
          borderSkipped: false,
          barPercentage: 0.55,
...
function initTimeChart() {
  ...
  // Manual legend
  const legend = document.querySelector('#timeChart').closest('.analytics-card').querySelector('.donut-legend') || null;
  // Render inline legend below chart container
  const wrap = ctx.closest('.analytics-card');
  if (wrap && !wrap.querySelector('.donut-legend')) {
...
function renderSkillScoreCards() {
  ...
  container.innerHTML = skillData.map(s => `
    <div class="skill-score-card">
      ...
      <div class="ssc-val ${s.tier}">${s.score}<span style="font-size:0.65em;font-weight:500;color:var(--gray-400)">%</span></div>
...
function initComparisonChart() {
  ...
  const labels = ['React', 'Node.js', 'JS Core', 'Algorithms', 'Databases', 'Security'];
...
function renderTimeline() {
  ...
  const events = [
    { type: 'start', icon: 'fa-solid fa-play',           time: '10:30 AM', title: 'Assessment Started',          sub: 'Full Stack Developer · 25 questions · 62 min' },
    { type: 'event', icon: 'fa-solid fa-code',            time: '10:38 AM', title: 'First coding question opened', sub: 'Q3 — twoSum · Algorithms · Hard' },
    { type: 'event', icon: 'fa-solid fa-video',           time: '10:55 AM', title: 'Video question answered',      sub: 'Q5 — System Design · 5:12 duration' },
    { type: 'event', icon: 'fa-solid fa-check-double',    time: '11:12 AM', title: 'Section: Databases',           sub: '3 questions completed' },
    { type: 'event', icon: 'fa-solid fa-chart-line',      time: '11:20 AM', title: 'Highest scoring streak',       sub: '8 consecutive correct answers' },
    { type: 'end',   icon: 'fa-solid fa-flag-checkered', time: '11:25 AM', title: 'Assessment Submitted',          sub: '54 min 38 sec · 7 min remaining' },
  ];
